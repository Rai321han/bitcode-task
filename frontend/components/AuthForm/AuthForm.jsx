...
export function FormHeading({ children, className, ...rest }) {
  return (
    <h1
      className={cn("text-2xl text-center font-bold text-gray-600", className)}
      {...rest}
    >
      {children}
    </h1>
...
export function Label({ children, className, ...rest }) {
  return (
    <label className={cn("text-sm text-gray-500", className)} {...rest}>
      {children}
    </label>
  );
}
...
export function FormInput({ children, className, ...rest }) {
  return (
    <input
      className={cn(
        "w-full text-sm text-gray-600 py-2 px-3 outline-2 outline-gray-200 rounded-md",
        className
      )}
      {...rest}
    />
...
export function FormButton({ className, children, disabled, ...rest }) {
  const disabledStyle = disabled ? "bg-gray-600 cursor-not-allowed" : "";
  return (
    <button
      disabled={disabled}
      type="submit"
      className={cn(
        "p-2 bg-orange-500 text-md rounded-md text-white mt-4 cursor-pointer shadow-sm border-b-2 border-b-orange-700 transition-colors duration-200 hover:bg-orange-700",
        className,
        disabledStyle
      )}
      {...rest}
